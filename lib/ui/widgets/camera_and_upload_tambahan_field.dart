...
class CameraAndUploadTambahanFieldForm extends StatefulWidget {
  final int index;
  ...
  @override
  State<CameraAndUploadTambahanFieldForm> createState() =>
      _CameraAndUploadTambahanFieldFormState();
}

class _CameraAndUploadTambahanFieldFormState
    extends State<CameraAndUploadTambahanFieldForm> {
  ...
  Future<void> _pickFile() async {
    ...
    // ✅ Nama asli file langsung dari galeri
    final fileName =
        result.files.single.name; // contoh: WhatsApp Image 20230827.jpg

    final now = DateTime.now();

    // Kalau mau copy ke direktori aplikasi dengan nama asli
    final newFile = await _copyFileToAppDir(originalFile, fileName);
    ...
    setState(() {
      ...
      _photoPosition = null;
      _isFileFromPicker = true;
      _displayController.text = fileName; // tampilkan nama asli
    });
            ...
            if (!_isFileFromPicker)
              Column(
                ...
                children: [
                  ...
                  // Text(
                  //   _dateTime != null ? _displayController.text : '-',
                  //   style: const TextStyle(fontSize: 12, color: Colors.grey),
                  // ),
