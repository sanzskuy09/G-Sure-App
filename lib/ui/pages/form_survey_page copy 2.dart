...
// map untuk controlle
  ...
  void _showConfirmationDialog(BuildContext context) {
    // 1. Ambil state saat ini langsung dari BLoC
    final blocState = context.read<FormBloc>().state;

    // 2. Lakukan pemisahan data dari state.formAnswers
    final Map<String, dynamic> jsonData = {};
    final Map<String, Map<String, dynamic>> fileData = {};

    // Loop melalui state.formAnswers, bukan variabel lokal
    for (final entry in blocState.formAnswers.entries) {
      final key = entry.key;
      final value = entry.value;
      if (value is Map && value.containsKey('file')) {
        fileData[key] = Map<String, dynamic>.from(value);
    ...
    // 3. (Tidak ada perubahan) Logika pemisahan dan format tetap sama
    const encoder = JsonEncoder.withIndent('  ');
    final String jsonString = encoder.convert(jsonData);
    final String fileString = encoder.convert(
      fileData.map(
        (key, value) => MapEntry(
          key,
          (value['file'] as File?)?.path ?? 'Path tidak ditemukan',
        ),
      ),
    );
    ...
    showDialog(
      ...
      builder: (dialogContext) {
        // Gunakan context baru untuk dialog
        return AlertDialog(
          title: const Text("Konfirmasi Pengiriman"),
          content: SingleChildScrollView(
            child: Column(
          ...
          actions: [
            ...
            FilledButton(
              child: const Text("Kirim"),
              onPressed: () {
                // 5. Ubah onPressed untuk mengirim event SubmitForm ke BLoC
                // Gunakan context utama (bukan dialogContext) untuk mengakses BLoC
